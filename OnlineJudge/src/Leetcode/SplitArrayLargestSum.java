...
public class SplitArrayLargestSum {
    ...
    public int splitArray(int[] nums, int m) {
    	...
    	for(int j = len; j < nums.length; j++) {
    		long tmp1 = tmp + nums[j] - nums[j-len];
    		tmp = tmp1;
    		res = Math.max(res, tmp1);
    	}
    	
        int max_len = (nums.length-(m-1));
        
        return (int)help(nums, m, 0, max_len);
    }
    
    
    public long help(int[] nums, int m, int begin_index, int max_len) {
    	if(m == 1) {
    		long res = 0;
    		for(int i = begin_index; i < nums.length; i++) {
    			res += nums[i];
    		}
    		return res;
    	}
  
    	for(int i = begin_index+1; (i < nums.length) && (i - begin_index <= max_len); i++) {
    		long tmp1 = 0;
    		...
    		long tmp = help(nums, m-1, i, max_len);

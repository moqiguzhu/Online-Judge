...
class RangeModule:
    ...
    def addRange(self, left: int, right: int) -> None:
        ...
        if len(self.d) == 0:
            self.d[left] = right
            return 
        idx1 = self.d.bisect_left(left)
        idx2 = self.d.bisect_left(right)
        print(idx1, idx2)
        
        if idx1 >= len(self.d):
            if self.d.peekitem(index = -1)[1] < left:
                self.d[left] = right
            else:
                e = self.d.popitem(index = -1)
                self.d[e[0]] = max(e[1], right)
            return
        else:
            # 情况好复杂
            pass
        item1 = self.d.peekitem(index = idx1)
        item2 = self.d.peekitem(index = idx2)
        if idx1 == idx2: # 还要考虑前一个元素
            if right == item1[0]:
                e = self.d.popitem(index = idx1)
                self.d[left] = e[1]
            else:
                self.d[left] = right
        else:
            ...
            if right == item2[0]:
                ...
                idx = idx1
                while idx < idx2:
                    self.d.popitem(index = idx1)
                    idx += 1
                self.d[left] = right

    def queryRange(self, left: int, right: int) -> bool:
        idx = self.d.bisect_left(left)
        item = self.d.peekitem(index = idx)
        if left == item[0] and right <= item[1]:
            return True
        else:
            if idx + 1 < len(self.d):
                item = self.d.peekitem(index = idx + 1)
